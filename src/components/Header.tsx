...
export const Header: React.FC<HeaderProps> = ({ cartCount, wishlistCount, currency, user, onOpenSearch, onOpenCart, onOpenWishlist, onOpenAdmin, onOpenAuth, onToggleCurrency }) => {
    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-xs">
      <div className="max-w-7xl mx-auto px-3.5 py-2.5 flex items-center justify-between gap-2 sm:gap-4">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-purple-700 via-purple-600 to-indigo-600 flex items-center justify-center text-white font-black text-lg shadow-sm shadow-purple-500/20"><span className="text-[#F0A717] ml-0.5">ص</span></div>
          <div className="flex flex-col"><div className="flex items-center gap-1 leading-tight"><span className="text-slate-900 font-extrabold text-base tracking-tight">التخفيض</span><span className="text-[#F0A717] font-black text-base">الصح</span></div><span className="text-[10px] text-slate-400 font-semibold tracking-wider">FASHION & DEALS</span></div>
        </div>

        <button onClick={onOpenSearch} className="flex-1 max-w-md hidden sm:flex items-center gap-2 bg-slate-100/80 hover:bg-slate-100 text-slate-400 px-3.5 py-2 rounded-full text-xs font-medium transition-all border border-slate-200/50"><Search className="w-4 h-4 text-slate-400" /><span>ابحث عن فساتين، أحذية، ملابس كاجوال، أو عروض...</span></button>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          {isAdmin ? <button onClick={onOpenAdmin} className="flex items-center gap-1 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300/80 px-2 py-1 rounded-lg text-xs font-bold transition-all active:scale-95"><ShieldCheck className="w-4 h-4 text-amber-600" /><span className="hidden sm:inline">الإدارة</span></button> : <button onClick={onOpenAuth} className="p-2 rounded-full text-slate-700 hover:bg-slate-100 active:scale-95 transition-all" aria-label="الحساب"><User className="w-5 h-5" /></button>}
        </div>
      </div>
    </header>
  );
