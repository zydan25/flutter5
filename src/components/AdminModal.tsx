...
export const AdminModal: React.FC<AdminModalProps> = ({
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      console.error('Flask customers load failed:', err);
      onShowToast(err?.message || 'تعذر تحميل العملاء من Flask', 'error');
    } finally {
  ...
  useEffect(() => {
    if (!isOpen || activeTab !== 'customers') return;
    void loadUsers();
  }, [activeTab, isOpen]);
  ...
  const handleDeleteUser = async (uid: string) => {
    ...
    try {
      ...
      onShowToast('تم حذف العميل بنجاح من قاعدة Flask', 'success');
    } catch (error: any) {
      console.error('Flask customer delete failed:', error);
      onShowToast(error?.message || 'تعذر حذف العميل من Flask', 'error');
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-slate-300">
                <span>الإدارة عبر خادم Flask</span>
                {activeTab === 'customers' && <span className="text-emerald-400 font-bold">متصل</span>}
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 px-6 py-2.5 bg-slate-100 border-b border-slate-200 overflow-x-auto no-scrollbar">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
              activeTab === 'orders' ? 'bg-purple-600 text-white shadow-xs' : 'bg-white text-slate-700 hover:bg-slate-200/80'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
              activeTab === 'customers' ? 'bg-purple-600 text-white shadow-xs' : 'bg-white text-slate-700 hover:bg-slate-200/80'
            }`}
          >
            <Users className="w-4 h-4" />
            <span>العملاء من Flask</span>
          </button>

          <button
            onClick={() => setActiveTab('products')}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
              activeTab === 'products' ? 'bg-purple-600 text-white shadow-xs' : 'bg-white text-slate-700 hover:bg-slate-200/80'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
              activeTab === 'trends' ? 'bg-purple-600 text-white shadow-xs' : 'bg-white text-slate-700 hover:bg-slate-200/80'
            }`}
          >
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
          ...
          {activeTab === 'orders' && (
            <div className="space-y-4">
              ...
              {orders.length === 0 ? (
                <div className="bg-white p-8 rounded-2xl text-center border border-slate-100 text-slate-500 text-xs">لا توجد طلبات واردة حتى الآن.</div>
              ) : (
                orders.map((ord) => (
                  <div key={ord.id} className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="font-black text-xs text-purple-700 font-mono">{ord.orderNumber}</span>
                      ...
                      <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-purple-50 text-purple-700 border border-purple-200">
                        {ord.status === 'awaiting_payment' ? 'بانتظار الدفع' : ord.status === 'payment_submitted' ? 'تم إرسال إثبات الدفع' : ord.status === 'preparing' ? 'جاري التجهيز' : ord.status === 'in_shipping' ? 'جاري الشحن' : ord.status === 'delivered' ? 'تم التوصيل' : 'ملغي'}
                      </span>
                    </div>
                    ...
                    <div className="text-xs text-slate-600 flex items-center justify-between border-y border-slate-100 py-2">
                      ...
                      <span className="font-black text-purple-700">الإجمالي: {safeFormatNumber(ord.total)} {ord.currency || 'ر.ي'}</span>
                    </div>

                    <div className="flex items-center justify-end gap-2 pt-1">
                      <button
                        onClick={() => onUpdateOrderStatus(ord.id, 'in_shipping')}
                        className="text-xs font-bold px-3 py-1.5 rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 transition-all"
                      >
                        بدء الشحن
                      </button>
                      <button
                        onClick={() => onUpdateOrderStatus(ord.id, 'delivered', true)}
          ...
          {activeTab === 'customers' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-sm text-slate-800">العملاء المسجلون في Flask</h3>
                  <p className="text-xs text-slate-500">بيانات العملاء من خادم المتجر، وليست من Firebase</p>
                </div>
                <button
              ...
              {isLoadingUsers ? (
                ...
                <div className="bg-white p-8 rounded-2xl text-center border border-slate-100 text-slate-500 text-xs">لا يوجد عملاء مسجلون حالياً في قاعدة البيانات.</div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {flaskUsers.map((u) => (
                    <div key={u.uid} className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex items-center justify-between">
                      <div>
                        <h4 className="text-xs font-bold text-slate-800">{u.firstName || 'عميل'} {u.lastName || ''}</h4>
                        <div className="flex items-center gap-1.5 text-[11px] text-slate-500 mt-1">
                          <Phone className="w-3 h-3 text-purple-600" />
                          <span className="font-mono">{u.phone}</span>
                        </div>
                        <span className="text-[10px] text-slate-400 block mt-0.5">{u.governorate || 'اليمن'}</span>
                      </div>
                      <button onClick={() => void handleDeleteUser(u.uid)} className="p-2 text-slate-400 hover:text-rose-600 rounded-xl hover:bg-rose-50 transition-all" title="حذف العميل">
          ...
          {activeTab === 'products' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between"><h3 className="font-bold text-sm text-slate-800">أصناف المتجر ({products.length})</h3></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {products.slice(0, 30).map((p) => (
                  <div key={p.id} className="bg-white p-3 rounded-2xl border border-slate-200 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-xs font-black text-purple-700 block mt-0.5">{safeFormatNumber(p.price)} ر.ي</span>
              ...
              <div className="space-y-3">
                {campaigns.map((c) => (
                  <div key={c.id} className="bg-white p-4 rounded-2xl border border-slate-200/80 flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold bg-purple-50 text-purple-700 px-2.5 py-1 rounded-xl">{c.daysLeft || 'نشط'}</span>
                  </div>
                ))}
              </div>
            </div>
