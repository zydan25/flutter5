...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, user, onClose, onLogin, onLogout, onShowToast }) => {
  ...
  const verify = async (event: React.FormEvent) => {
    ...
    try {
      ...
      onClose();
      onShowToast(result.user.isAdmin ? 'تم تسجيل الدخول إلى الإدارة بنجاح 👑' : `أهلًا بك يا ${result.user.firstName || 'عزيزي العميل'}!`, 'success');
    } catch (error: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={onClose}>
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 bg-purple-700 text-white">
          <div className="flex items-center gap-2"><User className="w-5 h-5" /><h3 className="font-extrabold">{user ? 'الملف الشخصي' : step === 'profile' ? 'إكمال بيانات الحساب' : 'الدخول برقم الهاتف'}</h3></div>
        ...
        <div className="p-6">
          {user ? (
            ...
            <form onSubmit={verify} className="space-y-3">
              ...
              <input required inputMode="numeric" maxLength={6} autoComplete="one-time-code" value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, '').slice(0, 6))} className="w-full text-center tracking-[0.4em] text-xl font-black p-3 rounded-xl border border-purple-200 bg-purple-50" placeholder="000000" />
              <button disabled={busy} className="w-full py-3 rounded-xl bg-purple-600 disabled:opacity-50 text-white font-bold text-sm">{busy ? 'جاري التحقق...' : 'تحقق من الرمز'}</button>
            ...
            <form onSubmit={submitProfile} className="space-y-3">
              <div className="rounded-xl bg-amber-50 border border-amber-100 px-3 py-2 text-xs text-amber-800">تم التحقق من رقمك. هذه الخطوة تظهر مرة واحدة فقط للحساب الجديد.</div>
              {profileFields}
              <button disabled={busy} className="w-full py-3 rounded-xl bg-purple-600 disabled:opacity-50 text-white font-bold text-sm">{busy ? 'جاري حفظ البيانات...' : 'حفظ البيانات وإنشاء الحساب'}</button>
            </form>
          )}
        </div>
      </div>
