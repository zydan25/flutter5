...
export async function completeProfileApi(payload: { firstName: string; secondName?: string; thirdName?: string; lastName?: string; governorate: string }): Promise<ApiUser> {
  return (await apiFetch<{ success: boolean; user: ApiUser }>('/takhfid/api/v2/auth/complete-profile', { method: 'POST', body: JSON.stringify(payload) })).user;
}
export async function verifyOtpApi(payload: { phoneNumber: string; otp: string; firstName: string; secondName?: string; thirdName?: string; lastName?: string; governorate: string }): Promise<VerifySessionResponse> {
  const result = await apiFetch<VerifySessionResponse>('/takhfid/api/v2/auth/verify-otp', { method: 'POST', body: JSON.stringify(payload) }); setAccessToken(result.accessToken); return result;
}
